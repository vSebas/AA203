...
discounted_return = torch.tensor(
    ...
    dtype=torch.float,)
naive_loss = -torch.stack([log_prob * discounted_return for log_prob in log_probs]).sum()
...
causal_loss = -torch.stack([log_prob * ret for log_prob, ret in zip(log_probs, returns)]).sum()

# 3) REINFORCE with causality trick and baseline to "center" the returns
baseline = returns.mean()
baseline_loss = -torch.stack([log_prob * (ret - baseline) for log_prob, ret in zip(log_probs, returns)]).sum()
...
Compared with the DQN run above, REINFORCE performed better after training.
The naive estimator was the noisiest, which makes sense because it gives every
action the same full-trajectory return. That means an action can get credit for
rewards that happened before it was even taken. The causal estimator fixes this
by using only the return-to-go after each action. The baseline version then
centers those returns, which reduced variance and gave the most stable behavior
in my runs. All three REINFORCE variants were still noisy, but the baseline
version was the most reliable.
